...
所有查询都带 user_id 过滤——对话是私有域，别人的消息一个字都不能漏出去。
...
class AiChatRepository:
    ...
    async def add_user_message(self, user_id: int, content: str) -> AiChatMessage:
        """记下用户这一句。"""
        message = AiChatMessage(user_id=user_id, role=ROLE_USER, content=content)
        self.session.add(message)
        await self.session.flush()
        return message

    async def add_assistant_message(
        self, user_id: int, content: str, actions: list[dict] | None = None
    ) -> AiChatMessage:
        """记下 AI 这一轮的回复（动作草案原样存，仅备查）。"""
        message = AiChatMessage(
            user_id=user_id, role=ROLE_ASSISTANT, content=content, actions=actions
        )
        self.session.add(message)
        await self.session.flush()
        return message

    async def list_recent(self, user_id: int, limit: int) -> list[AiChatMessage]:
        """取某用户最近 `limit` 条消息，**按时间正序**返回（对话就是要从头读）。
