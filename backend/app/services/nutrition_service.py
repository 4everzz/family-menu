...
#:
#: ⚠️ **`v1` 这个版本号是给"改写匹配逻辑"准备的**，别当装饰：
#:    缓存里存的是**已经挑好的结果**（比如 `猪肉 → 猪肉(肥) 816`）。
#:    一旦改了 `_pick_best_food` 的选择规则，**旧结果会继续被命中**，
#:    表现就是"改了代码却没生效"——实测踩过一次，排查了半天。
#:    TTL 是一天，等它过期太久；正确做法就是**改规则时把版本号 +1**，
#:    老 key 自然作废（不用手动清 Redis，也不会误删别的缓存）。
_CACHE_PREFIX = "fm:nutrition:v2:"
...
#: 成分表里表示"整体 / 通用"条目的括号标记。**顺序即优先级。**
#:
#: ⚠️ 真实事故（2026-09-21）：`search_food("猪肉")` 返回 20 条，其中
#:      「猪肉(肥) 816」「猪肉(瘦) 143」「猪肉(肥,瘦) 395」。
#:    旧规则在"带括号的候选"里**取名字最短的**——「猪肉(肥)」正好 6 个字，
#:    最短，于是选中**纯肥肉 816 kcal/100g**，
#:    把「红烧肉 500g」估成了 2120 kcal（实际约 500~800）。
#:
#: 为什么用"括号里是并列词"来认通用条目？
#:    因为这是成分表的**书写惯例**：`(肥,瘦)` 表示肥瘦都有 = 整体的代表值。
#:    这种**结构性信号**比"名字短"可靠得多——"名字短"和"更像用户要的东西"
#:    之间没有任何因果关系，它当初能 work 纯属巧合。
_GENERIC_BRACKET_MARKERS = ("肥,瘦", "肥瘦", "均值", "代表值", "鲜")
...
def _pick_best_food(foods: list[dict], wanted: str) -> dict | None:
    ...
    ── 教训 ────────────────────────────────────────────
    ⭐ 三次事故是同一个根因：**把"名字短"当成了"更像用户要的东西"**。
    前两次靠"候选词必须两个及以上字"绕开了单字特例，
    第三次（`猪肉`→`猪肉(肥)`）才彻底把它从"带括号候选"这一档里降级——
    改成先看有没有**结构性标记**（括号里是不是并列词），见 `_GENERIC_BRACKET_MARKERS`。
    ...
    ── 现在的规则（按优先级）────────────────────────────
      · 名字**完全等于**候选词                      → 采用（"豆腐"→"豆腐"）
      · 候选词**单个字**                            → **一律不采用**（宁可退化）
      · 前缀命中 + 括号里是**并列/通用标记**         → 采用 ⭐ 最可能是"整体食材"
        （`猪肉(肥,瘦)` / `牛肉(肥瘦)` / `某物(均值)`）
      · 前缀命中 + 其余带括号的规范名                → 取最短（退而求其次）
      · 前缀命中 + 普通后缀（`鲈鱼`→`鲈鱼(鲜)`）      → 取最短
      · 其余                                        → 不采用
      排除项：前缀后接"部位词"的（`鸡蛋`→`鸡蛋白`）一律跳过——那是零件不是整体。
    """
    if not foods:
        return None
    ...
    # ⭐ 档 1：带括号的里面，先按"通用标记"优先级找**整体条目**。
    #
    #    这一步是 2026-09-21 补的，补的原因见 _GENERIC_BRACKET_MARKERS 的注释：
    #    原来直接跳到"取最短名"，于是「猪肉(肥)」击败「猪肉(肥,瘦)」——
    #    只因为它少一个字符。把"名字长度"当相关性，是这里最深的那个坑。
    for marker in _GENERIC_BRACKET_MARKERS:
        for item in bracketed:
            if marker in str(item.get("name") or ""):
                return item

    # 档 2/3：没有通用标记，才退到"取最短名"（比乱猜好，但确实只是将就）
    #
    # ⚠️ 排序键带上名字本身，是为了**长度相同时有个稳定的先后**。
    #    `鸡蛋(白皮)` 和 `鸡蛋(红皮)` 都是 6 个字，只按 length 排的话
    #    `min()` 会返回列表里**先出现的那个**——结果就随上游返回顺序抖动：
    #    同样的查询今天给 138、明天给 156。这种不确定性最难查。
    for bucket in (bracketed, plain):
        if bucket:
            return min(bucket, key=lambda f: (len(str(f.get("name") or "")), str(f.get("name") or "")))
