...
function avatarText(name) {
  return (name || '微').slice(0, 1).toUpperCase();
}
  ...
  <section v-else class="card user-card">
    ...
    <div v-for="user in users" :key="user.id" class="user-row">
      <div class="user-main">
        ...
        <span v-else class="avatar avatar-fallback user-avatar">{{ avatarText(user.nickname) }}</span>
        <div class="user-copy">
...
.user-list-head {
  display: grid;
  grid-template-columns: minmax(220px, 1.4fr) minmax(170px, 1fr) minmax(150px, .8fr) 90px;
  gap: 14px;
  padding: 10px 16px;
